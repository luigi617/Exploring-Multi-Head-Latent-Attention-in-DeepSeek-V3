...
class MHA(nn.Module):
    """
    Standard Multi-Head Attention layer with rotary embeddings and KV caching.

    Attributes:
        dim (int): Model dimension.
        n_heads (int): Number of attention heads.
        n_local_heads (int): Number of local heads in distributed settings.
        qk_head_dim (int): Dimension of each query/key head.
        v_head_dim (int): Dimension of each value head.
    ...
    def __init__(self, args: ModelArgs):
        ...
        # Projection layers
        self.wq = ColumnParallelLinear(self.dim, self.n_heads * self.qk_head_dim)
        self.wk = ColumnParallelLinear(self.dim, self.n_heads * self.qk_head_dim)
        self.wv = ColumnParallelLinear(self.dim, self.n_heads * self.v_head_dim)
        self.wo = RowParallelLinear(self.n_heads * self.v_head_dim, self.dim)
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
        ...
        k_rope, k_nope = k.split([self.qk_rope_head_dim, self.qk_nope_head_dim], dim=-1)
        q_rotated = apply_rotary_emb(q_rope, freqs_cis)
        k_rotated = apply_rotary_emb(k_rope, freqs_cis)

        # Recombine rotated and non-rotated components
        q = torch.cat([q_nope, q_rotated], dim=-1)
        k = torch.cat([k_nope, k_rotated], dim=-1)
        ...
        context = torch.einsum("bsht,bthd->bshd", attn_weights, self.v_cache[:bsz, :end_pos])

        # Final projection
        return self.wo(context.flatten(2))
